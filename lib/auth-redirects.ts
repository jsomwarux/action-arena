...
const configuredPasswordResetRedirectUrl =
  process.env.EXPO_PUBLIC_PASSWORD_RESET_REDIRECT_URL?.trim();

export const passwordResetRedirectUrl =
  configuredPasswordResetRedirectUrl && configuredPasswordResetRedirectUrl.length > 0
    ? configuredPasswordResetRedirectUrl
    : Linking.createURL(PASSWORD_RESET_PATH, {
