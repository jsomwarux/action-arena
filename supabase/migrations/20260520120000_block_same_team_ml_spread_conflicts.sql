...
as $$
  ...
  select case
    ...
    when coalesce(p_left_market = p_right_market, false)
      ...
      end then 'direct_contradiction'
    when (
        (p_left_market = 'moneyline' and p_right_market = 'spread')
        or (p_left_market = 'spread' and p_right_market = 'moneyline')
      )
      and left_side <> ''
      and left_side = right_side then 'same_team_moneyline_spread'
    else null
  end
  from normalized
$$;
...
as $$
  ...
  select case
    when p_market = 'moneyline' and line is not null then
      'same-team moneyline and spread can''t be combined'
    when p_market = 'spread'
      and (
        (p_left_line is null and p_right_line is not null)
        or (p_left_line is not null and p_right_line is null)
      ) then
      'same-team moneyline and spread can''t be combined'
